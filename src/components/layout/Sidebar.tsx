...
const SECTIONS = [
  { label: "STEP 1  데이터 적재", items: STEP1, color: "text-blue-400" },
  { label: "STEP 2  표준화 파이프라인", items: STEP2, color: "text-violet-400" },
  { label: "STEP 3  AI 분석", items: STEP3, color: "text-emerald-400" },
  { label: "운영 관리", items: OPS, color: "text-slate-400" },
];

function NavLink({ href, icon: Icon, label, sub, active }: NavItem & { active: boolean }) {
  return (
    <Link
      href={href}
      className={cn(
    ...
    >
      ...
      <div>
        ...
        <div className={cn("text-xs", active ? "text-blue-200" : "text-slate-500 group-hover:text-slate-400")}>
          {sub}
        </div>
      </div>
    </Link>
    ...
    <aside className="w-64 bg-slate-900 text-white flex flex-col min-h-screen">
      ...
      <nav className="flex-1 p-3 space-y-4 overflow-y-auto">
        {SECTIONS.map(({ label, items, color }) => (
          <div key={label}>
            <p className={`text-xs font-bold px-3 mb-1 tracking-wide ${color}`}>{label}</p>
