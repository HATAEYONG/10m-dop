...
const WORD_STANDARDS = [
  ...
  /* ── YH ERP 표준 단어 (E업체 YH ERP ERD 기반) ── */
  { eng:"EMPLOYEE",      kor:"사원",      abbr:"EMP",     rule:"사원번호 emp_no — VARCHAR(20) · PAY/HRK 공통 PK",   example:"EMP_NO, EMP_NM" },
  { eng:"WORK_CENTER",   kor:"작업센터",  abbr:"WC",      rule:"wc_cd VARCHAR(10) · 공장코드(fac_cd)와 복합 관리",  example:"WC_CD, WC_NM" },
  { eng:"INSPECTION",    kor:"검사",      abbr:"IQC",     rule:"수입검사 iqc_no · 출하검사 oqc_no 접두어로 구분",   example:"IQC_NO, IQC_QTY_OK" },
  { eng:"FIXED_ASSET",   kor:"고정자산",  abbr:"FA",      rule:"관리번호 mng_no VARCHAR(50) — 자산 PK 기준",        example:"FA_MNG_NO, FA_GET_AMT" },
  { eng:"PAYROLL",       kor:"급여",      abbr:"PAY",     rule:"귀속연도 app_year CHAR(4) + 사원번호 복합키",        example:"PAY_APP_YEAR, PAY_AMT" },
  { eng:"FISCAL_YEAR",   kor:"귀속연도",  abbr:"YEAR",    rule:"CHAR(4) YYYY · PAY/PAZ 모듈 기준연도",             example:"APP_YEAR CHAR(4)" },
  { eng:"BUSINESS_NO",   kor:"사업자번호",abbr:"BIZ",     rule:"10자리 숫자 · 하이픈 포함 시 VARCHAR(20)",          example:"BIZ_NO VARCHAR(20)" },
  { eng:"UNIT_CODE",     kor:"단위코드",  abbr:"UM",      rule:"업무코드(bc suffix) · EA/KG/M/SET 등 표준코드표",   example:"UM_BC VARCHAR(10)" },
];
...
const DOMAIN_STANDARDS = [
  ...
  /* ── YH ERP 추가 도메인 표준 ── */
  { domain:"YH_AMOUNT",    type:"DECIMAL(18,5)",  len:"18자리 소수5",  nullable:true,  example:"OUT_AMT DECIMAL(18,5)",      rule:"YH ERP amt/qty 기본형 — Canonical 적재 시 DECIMAL(18,2)로 다운캐스트" },
  { domain:"YH_COST",      type:"DECIMAL(18,10)", len:"18자리 소수10", nullable:true,  example:"END_UP DECIMAL(18,10)",      rule:"YH ERP 단가(up) 전용 — 원가정밀도 유지, 표시 시 소수4자리 반올림" },
  { domain:"YH_YEAR",      type:"CHAR(4)",        len:"고정 4",        nullable:false, example:"APP_YEAR CHAR(4)",           rule:"YH PAY/PAZ 귀속연도 — Canonical DATE로 변환 시 YYYY-01-01" },
  { domain:"YH_PERIOD",    type:"CHAR(7)",        len:"고정 7",        nullable:false, example:"COST_MON CHAR(7)",           rule:"YH COS/LEB wrk_mon YYYYMM형 — Canonical YEAR_MONTH(VARCHAR(7)) 사용" },
  { domain:"YH_EMP_NO",    type:"VARCHAR(20)",    len:"최대 20",       nullable:false, example:"EMP_NO VARCHAR(20)",         rule:"YH PAY/HRK 사원번호 — PAY300은 VARCHAR(10)이므로 LEFT JOIN 시 주의" },
];
...
const CODE_STANDARDS = [
  ...
  /* ── YH ERP 코드 표준 ── */
  { group:"YH 재직 상태",  code:"YH_STAT_BC", values:[{v:"ACT",l:"재직"},{v:"LOA",l:"휴직"},{v:"RET",l:"퇴직"},{v:"TMP",l:"임시"}], tables:["HRK100"] },
  { group:"YH 급여 유형",  code:"YH_TY_BC",   values:[{v:"PAY",l:"지급"},{v:"DED",l:"공제"},{v:"ADJ",l:"조정"}], tables:["PAZ300"] },
  { group:"YH 단위 코드",  code:"YH_UM_BC",   values:[{v:"EA",l:"개"},{v:"KG",l:"킬로그램"},{v:"M",l:"미터"},{v:"SET",l:"세트"},{v:"BOX",l:"박스"}], tables:["DMA100","CAM100_YH","LEB100"] },
  { group:"YH 자산 상태",  code:"YH_STAT_FA", values:[{v:"ACT",l:"사용중"},{v:"DSP",l:"처분"},{v:"REP",l:"수리중"},{v:"RET",l:"폐기"}], tables:["FAT100"] },
  { group:"YH 검사 판정",  code:"YH_IQC_STS", values:[{v:"OK",l:"합격"},{v:"NG",l:"불합격"},{v:"CND",l:"조건부합격"},{v:"RTN",l:"반품"}], tables:["QMM100"] },
];
